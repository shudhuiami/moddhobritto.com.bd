...
export class categoriesComponent implements OnInit {
  ...
  getCategoryProducts = () => {
    this.Loading = true;
    this.apiService.category( this.category_slug, this.param).subscribe(response => {
      this.Loading = false;
  ...
  getCategories = () => {
    this.Loading = true;
    this.apiService.categorylist( this.category_slug, this.bodyParam).subscribe(response => {
      this.Loading = false;
      if (response.status === 2000) {
